...
- `lib/*.sh` - Module library (12 modules)
...
- **`lib/status.sh`** - Worker session status reporting
...
# Show worker session status (commits ahead, merge state)
clsecure --status
clsecure --status --all   # all workers, not just current project
