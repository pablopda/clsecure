...
# Exports: sanitize_mcp_config, sanitize_worker_claude_home_paths, check_worker_mcp_runtime, sanitize_hook_relative_paths
# 
# Usage:
...
# Sanitize relative "./" paths in project-local Claude hook commands.
# Claude Code's .claude/settings.json hooks may reference project-local scripts
# using "./" relative paths (e.g. "python3 ./.claude/tools/auto-format.py").
# These break when Claude Code changes CWD to a subdirectory during a session.
# We rewrite "./" to the absolute WORKER_PROJECT path so hooks work from any CWD.
sanitize_hook_relative_paths() {
    local settings_file="$WORKER_PROJECT/.claude/settings.json"

    [ -f "$settings_file" ] || return 0

    if ! command -v python3 &>/dev/null; then
        log_warn "Found .claude/settings.json but python3 is unavailable; skipping hook path sanitization."
        return 0
    fi

    log_step "Sanitizing hook relative paths (.claude/settings.json)..."
...
try:
    worker_project = os.environ["WORKER_PROJECT"]
    settings_path = os.path.join(worker_project, ".claude", "settings.json")
    ...
    # Pattern: "./" at start of string or after whitespace (but not "../")
    dot_slash_re = re.compile(r'(^|\s)\.\/')
    abs_prefix = worker_project.rstrip("/") + "/"
    changed = False

    for event_name, event_cfg in hooks.items():
        ...
        for entry in event_cfg:
            ...
            for hook in hook_list:
                ...
                if not isinstance(cmd, str):
                    continue
                if not dot_slash_re.search(cmd):
                    continue
                new_cmd = dot_slash_re.sub(
                    lambda m: m.group(1) + abs_prefix, cmd
                )
                if new_cmd != cmd:
